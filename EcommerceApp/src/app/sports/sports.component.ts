...
productService = inject(ProductsService);
  ...
  sportsCategoryId: string | null = null;

  ngOnInit() {
    this.loadCategories();
    this.loadBrands();
    this.loadProducts();
this.sportsCategoryId = this.route.snapshot.paramMap.get('categoryId'); 
 }
  ...
  loadProducts() {
    this.productService.getProducts().subscribe(
      (data) => {
        this.sportsProducts = data.filter(p => p.categoryId === this.sportsCategoryId);
        let filtered = this.filterProducts(this.sportsProducts);
        this.totalProducts = filtered.length;
        this.sportsProducts = this.paginateProducts(filtered);
      }
    );
  }
  ...
  onSort() {
    ...
    this.productService.getProducts().subscribe(
      (data) => {
        ...
        this.sportsProducts = this.paginateProducts(sorted);
      }
    );
  }
